...
from browser_storage import BrowserAppStorage
...
app = Flask(__name__)
app.secret_key = os.environ.get("FLASK_SECRET_KEY", load_secret_key())
app.config["MAX_CONTENT_LENGTH"] = 32 * 1024 * 1024


def get_storage() -> BrowserAppStorage:
    return BrowserAppStorage(APP_DIR)
...
def save_uploaded_file(storage: BrowserAppStorage, setting_key: str, uploaded_file) -> Path:
...
def download_output(filename: str):
    storage = get_storage()
    return send_from_directory(storage.output_dir, filename, as_attachment=True)
